 ...
 */
fun TextFieldBuffer.applyMarkdown(insertion: MarkdownInsertion) {
 ...
 * stacking `>> `.
 */
fun TextFieldBuffer.toggleLinePrefix(prefix: String) {
...
fun TextFieldBuffer.insertText(text: String) {
 ...
 */
fun TextFieldBuffer.deleteBackwards() {
    val start = selection.min
    val end = selection.max
    if (start != end) {
        delete(start, end)
    ...
    if (start == 0) return
    delete(previousGraphemeBoundary(asCharSequence().toString(), start), start)
}

/**
 * The index one grapheme cluster back from [index], or 0 if there is none.
 *
 * A seam because grapheme segmentation is a Unicode table, not an algorithm worth re-deriving: the
 * JVM has one in `java.text.BreakIterator` and every other platform has its own. Getting it wrong is
 * not subtle — see [deleteBackwards].
 */
internal expect fun previousGraphemeBoundary(text: String, index: Int): Int
 ...
 */
fun TextFieldBuffer.appendBlock(block: String) {
...
fun TextFieldBuffer.removeBlock(block: String) {
...
const val MARKDOWN_LINK_SUFFIX = "](https://)"
const val MARKDOWN_LINK_CARET = 2
 ...
 */
fun TextFieldState.editFromViewModel(block: TextFieldBuffer.() -> Unit) {
    edit(block)

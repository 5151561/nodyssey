...
class NodeSeekJsonClient(
    ...
    /**
     * The headers the site's own XHRs carry. [origin] is set on writes only, which is what a browser
     * does — a same-origin GET carries no `Origin`.
     */
    private fun xhrRequest(
        path: String,
        referer: String,
        method: String = "GET",
        body: HttpBody? = null,
        ...
        HttpRequest(
            url = NodeSeekSite.absoluteUrl(path) ?: error("Invalid path: $path"),
            method = method,
            headers =
            buildMap {
                ...
                if (origin) put("Origin", NodeSeekSite.BASE_URL)
                putAll(extraHeaders)
            },
            body = body,
        )

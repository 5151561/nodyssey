...
class YabsReportParserTest {
    ...
    private val report =
        checkNotNull(QualityReportParser.parse(Fixtures.load("reports/nodequality-yabs.txt")))
    ...
    fun `reads both of the tables fio puts in one section`() {
        ...
        assertTrue(labels.none { it.isEmpty() || it.all { c -> c == '-' } }, "separator survived: $labels")
    }
}

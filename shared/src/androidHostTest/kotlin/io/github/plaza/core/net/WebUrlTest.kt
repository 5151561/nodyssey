...
class WebUrlTest {
    ...
    fun `reads every part the way java net URI read it`() {
        urls.forEach { url ->
            ...
            assertEquals(expected.scheme.lowercase(), actual.scheme, "$url scheme")
            assertEquals(expected.userInfo, actual.userInfo, "$url userInfo")
            assertEquals(expected.host, actual.host, "$url host")
            assertEquals(expected.port, actual.port, "$url port")
            assertEquals(expected.path, actual.path, "$url path")
            assertEquals(expected.rawQuery, actual.rawQuery, "$url rawQuery")
            assertEquals(expected.fragment, actual.fragment, "$url fragment")
        }
    }
    ...
    fun `refuses everything java net URI refused to parse as an absolute url`() {
        rejected.forEach { url ->
            ...
            assertEquals(true, jvmRefused, "$url should be refused by java.net.URI too")
            assertNull(WebUrl.parse(url), url)
        }
    }

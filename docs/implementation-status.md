...
| 能力 | 当前实现 |
|---|---|
| 首页与阅读 | 版块、排序、Paging 追加、下拉刷新、Room 离线缓存、已读标记；帖子正文与评论原生 Compose 渲染。点底栏已选中的「首页」或顶栏应用名回列表顶部，同时把随滚动隐藏的底栏推回来。帖子读到的页与楼层记在本地（`ReadingPositionStore`，DataStore，条数跟随「浏览历史」的保留上限），下次打开由跳页面板的「上次阅读」原位返回 |

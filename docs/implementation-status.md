...
更新日期：2026-09-14 · 代码基线：当前工作区（1.2.21 之后，评论统一使用普通楼中楼预览）
...
| 楼中楼回复 | 固定使用普通模式，无需设置开关，按原楼层顺序显示；有直接回复时在点赞、鸡腿同一操作栏显示首页同款实心评论图标、数量和展开箭头，按钮点击反馈居中包住整组内容；点击后展开头像、作者、楼层和正文卡片，点卡片跳到原楼层，底部可收起。只计算已加载分页中的直接回复，数量随分页更新。楼层跳转、通知和发布后的定位沿用现有流程；正文、表态、回复、引用、编辑和屏蔽规则复用原详情页 |

...
| 通知与私信 | 「互动」/ 私信两组、未读 badge、全部已读、楼层跳转；私信列表、会话、Markdown 与发送；长按气泡复制或引用该条（引用把它写成 Markdown 引用块放进草稿，站点会话本身没有这个动作）。**站点的 @我 与 回复主题 在 App 里合并成一组**：站点「回复」按钮会在正文开头写 `@某人 #楼层`，同一条评论因此同时进两个列表，合并按 `comment_id`（没有就按 帖子+楼层）去重，标为已读会同时清两边，badge 也据此扣掉重复计数。通知行与私信列表都直接显示正文摘要（Markdown 压成一行），图片与表情用 `[图片]`/`[表情]` 占位——都是 App 增强，站点两处都没有 |

...
| 楼中楼回复 | 固定使用普通模式，无需设置开关，按原楼层顺序显示；作者栏的「回复给」入口在正文上方预览原消息，下方操作栏的实心评论图标、数量与箭头展开后续直接回复，两处独立收起。卡片展示头像、作者、楼层和正文，点卡片跳到原楼层；引用原文仅预览开头，未加载时保留楼层跳转入口。只统计已加载分页中的直接回复，数量随分页更新。楼层跳转、通知和发布后的定位沿用现有流程；正文、表态、回复、引用、编辑和屏蔽规则复用原详情页 |

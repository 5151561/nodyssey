...
| 通知与私信 | 「互动」/ 私信两组、未读 badge、全部已读、楼层跳转；私信列表、会话、Markdown 与发送；长按气泡复制或引用该条（引用把它写成 Markdown 引用块放进草稿，站点会话本身没有这个动作）。**站点的 @我 与 回复主题 在 App 里合并成一组**：站点「回复」按钮会在正文开头写 `@某人 #楼层`，同一条评论因此同时进两个列表，合并按 `comment_id`（没有就按 帖子+楼层）去重，标为已读会同时清两边，badge 也据此扣掉重复计数。私信列表的摘要把 Markdown 压成一行，图片与表情用 `[图片]`/`[表情]` 占位——App 增强，站点没有。**通知行不显示评论正文**：`at-me`/`reply-to-me` 的每一行只有 `id / viewed / comment_id / floor_id / created_at / commenter_id / title（帖子标题）/ post_id / first_comment_id / commenter_name`（2026-09-07 真机实测），接口不发正文，站点自己的通知页也不显示；正文只存在于帖子页，一条通知一次整页加载，列表加载做不起 |
...
| 图片 | 原生查看器、缩放、保存与系统分享 |
| 通知轮询 | WorkManager 周期轮询、15/30/60 分钟、仅 Wi-Fi、免打扰与 Android 通知渠道。渠道与开关按 App 的两组走（互动 / 私信）而不是站点的三组：同一条评论进两个列表，一个渠道一条通知就会弹两次；两组计数同时上涨的那次轮询会顺手取一遍合并列表，用重复数把「新增几条」算准 |
...
收藏接口只给标题：板块、作者、回复数、发帖时间都不在 `list-collection` 的返回里（候选字段名是按
其它端点猜的，真实返回还没抓过）。于是 App 自己记：`collected_post_meta` 表存站点在别处说过的话，
打开一篇已收藏的帖子时用页面写入（走得最多的一条，列表在使用中自己长完整），按下星标时从
feed 行 / 帖子楼主楼 / 读标记里各取所知合并写入，离线下载完成时用抓到的页补写，收藏列表每次加载
把接口确实给了的字段存下来。头像存页面给的 URL 加 uid 两列，取时页面地址优先、`/avatar/<uid>.png`
兜底——离线下载是按页面那个 URL 存的文件，断网时只有问同一个字符串才拿得到图。读的时候站点优先、本地填空，每一列可空、写入只补不盖。
早年在网页端收藏、App 从没打开也没下载过的帖子，行里仍然只有标题——这条路补不了，只有下载能。

**收藏的离线下载**是纯本地能力，站点不提供任何离线接口。整篇帖子（正文 + 全部回复 + 图片）落进
`offline_threads` / `offline_comments` / `offline_images` 三张表和 `filesDir/offline/images`，
队列本身也是表里的行，WorkManager 只负责“把队列排干”，所以进程被杀或 Wi-Fi 明天才来都不丢东西。
刻意**不复用 `post_details`**：那是个窗口缓存，在线打开一篇已下载的帖子就会把窗口以外的页删掉，
拿它冒充「已离线」会是一个下次刷新就毁约的说法。两边只在一处相接——请求失败且原因是“连不上站点”时，
`OfflineFirstPostRepository` 从离线表取同一页写进 `post_details`，用的是**下载当时**的时间戳，
所以缓存不会把上周的页当成刚拉的。站点明确拒绝（未登录、等级不够、帖子没了）不走这条路。
图片按 URL 的 SHA-256 存文件，Coil 侧一个拦截器排在流量策略之前把 URL 换成本地文件。
「离线版落后 N 条回复」靠收藏列表每次加载时把站点的回复数递给引擎；自动补新回复是每天一次的
周期任务，只从上次存到的那一页往后拉。细节见 `docs/design-implementation.md` 的 i1 章节。

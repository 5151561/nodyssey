...
| 通知与私信 | 「回复与@我」/ 私信两组、未读 badge、全部已读、楼层跳转；私信列表、会话、Markdown 与发送；长按气泡复制或引用该条（引用把它写成 Markdown 引用块放进草稿，站点会话本身没有这个动作）。**站点的 @我 与 回复主题 在 App 里合并成一组**：站点「回复」按钮会在正文开头写 `@某人 #楼层`，同一条评论因此同时进两个列表，合并按 `comment_id`（没有就按 帖子+楼层）去重，标为已读会同时清两边，badge 也据此扣掉重复计数。通知行与私信列表都直接显示正文摘要（Markdown 压成一行），图片与表情用 `[图片]`/`[表情]` 占位——都是 App 增强，站点两处都没有 |
...
| 图片 | 原生查看器、缩放、保存与系统分享 |
| 通知轮询 | WorkManager 周期轮询、15/30/60 分钟、仅 Wi-Fi、免打扰与 Android 通知渠道。渠道与开关按 App 的两组走（回复与@我 / 私信）而不是站点的三组：同一条评论进两个列表，一个渠道一条通知就会弹两次；两组计数同时上涨的那次轮询会顺手取一遍合并列表，用重复数把「新增几条」算准 |

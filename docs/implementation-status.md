...
| 应用内更新 | 关于页显示检查结果、新版本号、更新说明和包大小；下载进度可取消，下完交给系统安装器确认。冷启动静默查一次，六小时内不重复问 GitHub；发现新版时在「我的」的设置行与设置页的关于行上点红点，并在启动后弹一次提醒（版本号、包大小、可滚动的更新日志，「下载并安装」直接开下并进关于页，「稍后」后该版本不再提醒）；「设置 › 关于 › 启动时检查更新」可关掉启动检查与提醒，关于页的手动检查不受影响。只认本项目 `releases/latest` 的签名 APK，不走任何第三方中转；查不到就在按钮旁写清是连不上、GitHub 返回了几还是读不懂回答，更新卡里另有「在 GitHub 查看」这条自己去下载的出口 |

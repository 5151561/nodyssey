...
| 应用内更新 | 关于页显示检查结果、新版本号、更新说明和包大小；下载进度可取消，下完交给系统安装器确认。冷启动静默查一次，六小时内不重复问 GitHub；发现新版时在「我的」的设置行与设置页的关于行上点红点，并在启动后弹一次提醒（版本号、包大小、可滚动的更新日志，「下载并安装」直接开下并进关于页，「稍后」后该版本不再提醒）；「设置 › 关于 › 启动时检查更新」可关掉启动检查与提醒，关于页的手动检查不受影响。「设置 › 关于 › 接收 dev 版更新」默认关，打开后连 `vX.Y.Z-dev.N` 的测试包一起查（改读 releases 列表并按版本号取最大的），更新卡与启动提醒会标明「dev 测试版」并写清可能不稳定；关掉后只推正式版，已经装上的 dev 版不会被回退。检查更新与日志都读本项目 `updates` 分支上的静态清单（`stable.json` / `dev.json` / `changelog.json`，经 GitHub Pages 发布），不碰 api.github.com 的匿名配额；下载完按清单里的 sha256 校验再交给系统安装器。只认本项目 Releases 上的签名 APK，不走任何第三方中转；查不到就在按钮旁写清是连不上、GitHub 返回了几还是读不懂回答，更新卡里另有「在 GitHub 查看」这条自己去下载的出口 |

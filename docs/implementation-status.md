...
| 用户与账号 | 我的主页、公开用户空间、主题/评论/收藏；账号设置二级页全部接入站点 `/setting` 契约——资料字段读写、头像上传、修改密码、2FA 状态与开启、邮箱与 Telegram 绑定状态、解绑、远程偏好、屏蔽列表与解除屏蔽 |
...
## 已有界面，但数据或写操作未接入（10 项）

按用户可感知能力去重；同一业务内的读写方法不重复计数。生产实现会明确显示未接入、禁用操作或保留未知值，不伪造成功和实时数据。

| 领域 | 数量 | 能力 | 当前情况 |
|---|---:|---|---|
| 编辑与详情 | 1 | 评论发布 | 编辑器、引用与草稿完整；`LocalCommentComposerRepository.publish` 明确失败，真实发布端点尚未接入 |
| 编辑与详情 | 1 | NodeImage 上传 | 选择、队列、进度与失败态已完成；`NodeImageUploader` 尚无真实上传请求 |
...
| 关于与社区 | 1 | 社区注册人数 | 界面标注“每日更新”，当前数字仍是资源中的静态快照，没有真实数据源 |
| **合计** | **10** |  |  |
...
2026-07-28 当前工作区快照为 409 个 JVM/Robolectric 测试；spotless、单测、lint、assemble

...
再次点击操作栏的评论图标、数量或箭头可收起直接回复并返回原评论，上方的「回复给」独立控制。两个方向均默认收起，预览不递归展开更深层回复。
...
| 直接回复 | `ReactionRow`、`QuietReaction`、`CommentRepliesList`、`OutlinedCard` | 有直接回复时在操作栏显示实心评论图标、数量与箭头；默认收起，展开后显示卡片，正文复用 `PostRichContent`，头像和屏蔽规则沿用详情页 |
...
普通模式的回复数只包含已加载分页中可确认的直接回复，不包含间接回复，也不估算未加载页面的回复。

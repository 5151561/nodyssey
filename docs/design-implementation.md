...
评论统一采用普通模式，无需设置开关。参考 Fluxdo 的 `lib/widgets/post/post_item/widgets/post_replies_list.dart`：按原楼层顺序阅读，
...
底部「收起回复」关闭预览并返回原评论。预览不递归展开更深层回复。
...
回复关系由 `:shared` 的 `core/CommentReplies.kt` 从现有 `PostContent` 生成，只保存直接回复的原始评论索引。只接受开头段落或开头引用块中的
明确楼层引用，且实际链接属于当前站点、当前帖子，标签与链接楼层一致、父楼层早于当前楼层。
单独 @提及、正文中途引用、同段多个不同引用、缺失或重复楼层不计入回复预览；置顶评论保持独立显示。
这是一种基于已加载内容的归组方式，无法还原没有明确引用的回复关系。

普通模式的回复数只包含已加载分页中可确认的直接回复，不包含间接回复，也不估算未加载页面的回复。
继续分页后，已展开的卡片和数量一并更新；展开状态按稳定评论键保存，加载前页不会把展开状态移给另一楼层。
多页帖子先显示当前缓存里的关系，父楼层加载后再归组。分页沿用详情仓库和 Room 缓存。

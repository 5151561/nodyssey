...
按原楼层顺序阅读，
直接回复默认收起，点操作栏中的评论图标与数量后在原评论下展开预览卡片。回复入口独立靠左，点赞、鸡腿等操作靠右，中间留白；
复用首页的 `PlazaIcons.ModeComment` 实心图标及现有 `QuietReaction` 按钮，不加边框或固定底色，以小箭头表示展开或收起。
按钮保留 Material 3 的对称内边距，图标、数字和箭头统一垂直居中，让点击时的椭圆反馈包住整组内容；
通过整体偏移对齐正文左边缘，并将腾出的宽度留给右侧操作。窄屏下右侧操作换行时，回复入口仍与第一行对齐。
卡片含头像、作者、楼层和回复内容，点击后跳到原楼层；
再次点击操作栏的评论图标、数量或箭头可收起直接回复并返回原评论，上方的「回复给」独立控制。两个方向均默认收起。
**回复卡自己也带一个「N 条回复」**：A 的 #4 下面列 B 的 #7，#7 这张卡上再点开才是 A 回 #7 的 #10，一层一层往下走。
不在 #4 那一层把整棵子树拉平 —— 热帖里 #1 往往是整段讨论的根，拉平就是半个帖子。嵌套到 `MAX_REPLY_DEPTH` 层为止，
再往下「N 条回复」改成跳到原楼层，那条楼层在主列表里自己又是第 0 层，可以接着展开；上限的存在是因为每一层都要吃掉
一个头像加两侧内边距的宽度。
正文和操作沿用 Nodyssey 的 Material 3 主题、字号和楼层号。

| 交互 | 实现 | 行为 |
|---|---|---|
| 回复目标 | `CommentReplyTargetButton`、`CommentReplyTargetPreview`、`CommentPreviewContent` | 作者栏入口展开正文上方的「回复给」卡片；点击卡片跳到目标楼层，独立收起 |
| 直接回复 | `ReactionRow`、`QuietReaction`、`CommentRepliesList`、`OutlinedCard` | 有直接回复时在操作栏显示实心评论图标、数量与箭头；默认收起，展开后显示卡片，正文复用 `PostRichContent` 的只读模式，头像和屏蔽规则沿用详情页 |
| 逐层展开 | `CommentRepliesList` 自递归、`MAX_REPLY_DEPTH` | 回复卡上带它自己的直接回复数，点开嵌套一层；到上限后该按钮改为跳到原楼层 |
| 回复预览跳转 | `PostDetailScreen.jumpToFloor` | 点击卡片和卡片内的内容都定位到这条回复的原楼层，分页与阅读位置沿用现有流程 |
| 评论列表 | `ThreadList`、`CommentRow` | `LazyColumn` 按原始评论顺序显示，操作栏用 `FlowRow` 在窄屏换行 |

详情 UI 文件位于 `ui/src/commonMain/kotlin/io/github/nodyssey/ui/postdetail/`。

回复关系由 `:shared` 的 `core/CommentReplies.kt` 从现有 `PostContent` 生成，保存回复目标楼层、已加载目标的原始评论索引和直接回复索引，不复制正文。只接受开头段落或开头引用块中的
明确楼层引用，且实际链接属于当前站点、当前帖子，标签与链接楼层一致、父楼层早于当前楼层。
单独 @提及、正文中途引用、同段多个不同引用、当前评论缺失楼层或重复楼层不生成关系；目标楼层重复时也不猜测引用对象。
置顶评论保留「回复给」入口，但保持独立显示，不加入其他评论的直接回复卡片。
这是一种基于已加载内容的归组方式，无法还原没有明确引用的回复关系。

每一处的回复数都只算**那一层的直接回复**：#4 的按钮说 1 条（#7），#7 的卡也说 1 条（#10），没有哪个数字是子树总数。
都只包含已加载分页中可确认的关系，不估算未加载页面的回复。
继续分页后，已展开的卡片和数量一并更新；两个方向的展开状态分别按稳定评论键保存，加载前页不会把展开状态移给另一楼层。
多页帖子先显示当前缓存里的关系。目标楼层尚未加载时仍显示楼层入口，点击沿用现有跳转加载；目标与当前评论都在已加载范围内时可直接展开预览。
分页沿用详情仓库和 Room 缓存，预览内容随已加载原文更新。

引用跳转、通知定位、上次阅读和发布后定位均直接到达原始楼层。跨帖引用走正常链接导航。
编辑目标的页码仍取原评论的 `commentPages`。
...
- 返回键在输入框的 `leadingIcon` 里，不另起一条顶栏：搜索从首页顶栏进、压在首页栈上，
  是二级页面（和画板一致）；早期它是底部导航的一级 Tab，那时才没有返回键。
  帖子 Tab **不显示计数**，`/search` 不返回总数，已加载行数不是总数。
...
- **挂件外面不套容器。** 稿子把验证画成一个带边框的填充条，但 Cloudflare 的挂件自己就画一个
  带边框的填充盒子，外面再套一层就是框中框。所以这里只剩布局：挂件占满宽度，
  下面只在 App 确实有话要补时才加一行说明（已过期 / 未接入），那一行走的是表单的辅助文字语气。

...
| 画板 | 实现状态 | 主要代码 | 仍需注意 |
|---|---|---|---|
| f1 关于与社区 | 已按稿重做 | `ui/settings/AboutCommunityScreen.kt`、`ChangelogScreen.kt`、`Navigation.kt` | 更新状态有“最新/新版”两态；当前按钮打开 GitHub Releases，不伪造检查结果 |
...
| f3 Telegram 绑定 | 已接入账号联系方式流程 | `ui/account/ContactScreen.kt`、`ContactViewModel.kt`、`data/account/AccountSettingsRepository.kt` | 站点绑定过程仍受实际 Bot/站点返回状态约束 |
...
- 同一滚动页保留“本软件 → 社区与联络”的两屏节奏，进入社区段后标题切为“关于 · 社区”。

...
- [ ] 五套 × 亮暗 × 11 项 = 110 个值齐全,没有沿用另一套的
- [ ] 对比度沿用 j2 标准:所有 `on-` 配对 ≥4.5:1,正文 ≥7:1
- [ ] 中性面阶梯 §2.1 的单调性成立,亮暗方向相反
- [ ] `--ionsf` / `--ip` 落在 `--isf` 上都 ≥4.5:1
- [ ] 镜音双子的 `--sec` / `--ter` 有一句明确说明(跟不跟 container 同色相)
- [ ] 比例原则不破:这 11 个基本都在中性面和反色上,不应该把角色色再往外铺

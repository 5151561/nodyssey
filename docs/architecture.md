...
| 评论关系 | 从详情流中的 `PostContent` 派生 `CommentReplies` | 按原评论索引保存直接回复的索引列表，供展开预览使用，不复制正文 |
...
回复展开状态是 Compose 的可保存显示状态，使用稳定评论键关联，分页追加或前插不改变它们的归属。
`CommentReplies` 只建立直接回复关系，不计算嵌套深度或后代数量。主列表按原评论顺序显示正文，数量仅统计

...
阅读侧另有楼中楼回复预览、Room 离线缓存与已读标记、本机浏览历史与每帖阅读位置、图片全屏预览 / 保存 / 分享、
WorkManager 通知轮询，以及应用内检查更新与安装。

评论按原楼层顺序显示。点评论操作栏中的实心评论图标与数量展开直接回复卡片，再次点击同一按钮收起，点卡片跳到原楼层。
回复卡上还带它自己的回复数，可以顺着 #4 → #7 → #10 一层层点下去；嵌套到上限后改为跳到原楼层继续。
评论作者栏还提供「回复给」入口，在正文上方预览被回复的原消息；例如 B 回复 A、C 回复 B，B 可同时展示上方的 A 和下方的 C，两处独立展开、收起。
回复关系根据已加载评论开头的同帖楼层引用整理，每个数字只算那一层的直接回复，随分页载入更新。
引用目标位于当前已加载范围内时可直接预览，范围外保留楼层跳转入口。

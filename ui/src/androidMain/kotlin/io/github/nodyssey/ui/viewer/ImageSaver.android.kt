...
actual fun rememberImageGallerySaver(dispatchers: AppDispatchers): ImageGallerySaver {
    // The application context, not the activity's: the write outlives the screen by design — see
    // `ImageViewerViewModel` for why it is not launched from the composition's scope.
    val context = LocalContext.current.applicationContext
    return remember(context, dispatchers) { AndroidImageGallerySaver(context, dispatchers) }
}

private class AndroidImageGallerySaver(
    private val context: Context,
    private val dispatchers: AppDispatchers,
) : ImageGallerySaver {
    override suspend fun save(url: String): SaveOutcome = saveImageToGallery(context, url, dispatchers)
}

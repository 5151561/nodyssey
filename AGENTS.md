...
One application module, two library modules, and a desktop probe. `:app` is the NodeSeek application (Nodyssey): production Kotlin lives under `app/src/main/java/io/github/nodyssey/`, where `ui/` contains Compose screens, view models, and navigation, `platform/` holds the Android shells behind `data`'s interfaces, and `data/` is what is left of the repositories after step A7 — sixteen files, every one of them either a platform shell (WorkManager, the filesystem, Coil's caches, `java.net` failure classification) or the image-host protocol layer. The site's URL vocabulary, its CSS selectors, its parsers, the domain model, the network layer, the Room schema and every repository are no longer here — they moved to `:shared`, under those same package names.
...
`:designsys` and `:shared` hold what is not specific to any one forum, and **neither can see an application module** — the dependency runs one way only, so anything site-specific belongs in the app that knows it:

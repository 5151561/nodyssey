...
`:bbs1` is the second application: a multi-instance client for the bbs1org forum software, where the user adds sites by domain and the data comes from the site's JSON API plugin ([plaza-bbs1org](https://github.com/5151561/plaza-bbs1org)) rather than scraped HTML. Today it holds instance management (adding a site probes its `meta` endpoint first), browsing — forum filter, topic feed, thread detail with replies — and writing: password login per site, opening a thread, and replying. `net/` speaks the plugin's `?a=api&r=<resource>` envelope; post bodies are Markdown rendered client-side through `:core`'s parser and `:designsys`'s `RichContent`. The credential is a bearer token that reaches every screen as part of the `ForumInstance` it belongs to, and is sent on reads as well as writes because the server answers `can_post`/`can_reply` per identity; a 401 drops it through `InstanceRepository.authed`, and every screen watching the instance re-reads as an anonymous one. It is stored in its own DataStore file rather than in the site list, and that split is a backup boundary — `bbs1_data_extraction_rules.xml` excludes `sessions.preferences_pb` by name so a restored backup carries the user's sites but not their logins. Search, notifications, and editing or deleting one's own posts are the next slices — the plugin already serves all three. The two apps deliberately do **not** share a `Site` abstraction — their domain models are too different — so `:bbs1` owns its model/data/ui the same way `:app` does, and anything that turns out to be genuinely generic moves down into the library modules instead of sideways between apps. Its identifiers (`io.github.bbs1`, launcher name, icon) are placeholders; settle the branding before its first release tag.

...
`:bbs1` is the second application: a multi-instance client for the bbs1org forum software, where the user adds sites by domain and the data comes from the site's JSON API plugin ([plaza-bbs1org](https://github.com/5151561/plaza-bbs1org)) rather than scraped HTML. Today it holds instance management (adding a site probes its `meta` endpoint first), read-only browsing — forum filter, topic feed, thread detail with replies — with `net/` speaking the plugin's `?a=api&r=<resource>` envelope; post bodies are Markdown rendered client-side through `:core`'s parser and `:designsys`'s `RichContent`. Login and write operations are the next slices. The two apps deliberately do **not** share a `Site` abstraction — their domain models are too different — so `:bbs1` owns its model/data/ui the same way `:app` does, and anything that turns out to be genuinely generic moves down into the library modules instead of sideways between apps. Its identifiers (`io.github.bbs1`, launcher name, icon) are placeholders; settle the branding before its first release tag.

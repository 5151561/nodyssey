...
One application module, two library modules, and a desktop probe. `:app` is the NodeSeek application (Nodyssey): production Kotlin lives under `app/src/main/java/io/github/nodyssey/`, where `data/` owns the repositories, `platform/` holds the Android shells behind `data`'s interfaces, and `ui/` contains Compose screens, view models, and navigation. The site's URL vocabulary, its CSS selectors, its parsers, the domain model, the network layer and the whole Room schema are no longer here — they moved to `:shared`, under those same package names.

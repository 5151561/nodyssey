...
class QualityReportParserTest {
    ...
    fun `takes good and bad from the colour, not from the tick`() {
        ...
        assertEquals("a red ✔ is not good news", QualityReport.Tone.Bad, items[0].tone)
        assertEquals("a green ✘ is good news", QualityReport.Tone.Good, items[1].tone)
    }
     ...
     */
    @Test
    fun `plain green is the value ink, not a verdict`() {
        val section =
            coloured(
                "一、基础信息",
     ...
     */
    @Test
    fun `reads the verdict at the end of a bar, not the bar`() {
        val section =
            coloured(
                "三、风险评分",

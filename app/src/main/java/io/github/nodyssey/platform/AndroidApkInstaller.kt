...
class AndroidApkInstaller(
    ...
    /**
     * Not on the [ApkInstaller] interface: an `Intent` is Android's, and the screen that reaches this
     * one does so through `rememberInstallPermissionRequest`, which is where the activity result that
     * answers it is handled. Kept here so the address is stated once.
     */
    ...
    /**
     * Writes the file at [apkPath] into an install session and commits it.
     *
     ...
     */
    override suspend fun install(apkPath: String): Boolean =
